...
class PianoBox(nn.Module):
    ...
    def forward(self, x, h_init=None):
        ...
        attn_vec = F.softmax(attn_vec.reshape([-1, 12])).reshape([-1, 12, 1])
        x4 = attn_vec*x4
        x_out = F.relu(x4.sum(axis=1))
        group_prob = F.softmax(self.group_cls(x_out))
        pitch_prob = F.softmax(self.pitch_cls(x_out))
        chord_prob = F.softmax(self.chord_cls(x_out))
